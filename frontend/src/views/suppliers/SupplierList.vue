<template>
  <div class="page-card">
    <div class="table-topbar">
      <div class="left">
        <el-input v-model="query.keyword" placeholder="搜索供应商名称/编码/联系人" clearable style="width:260px" @keyup.enter="load(1)" @clear="load(1)">
          <template #prefix><el-icon><Search /></el-icon></template>
        </el-input>
        <el-select v-model="query.category" placeholder="类型" clearable style="width:140px" @change="load(1)">
          <el-option v-for="(t, k) in SUPPLIER_CATEGORY" :key="k" :label="t" :value="k" />
        </el-select>
    ...
    <el-table :data="list" v-loading="loading" stripe @selection-change="onSelect">
      ...
      <el-table-column label="类型" width="110">
        <template #default="{ row }">{{ dictText(SUPPLIER_CATEGORY, row.category) }}</template>
      </el-table-column>
      <el-table-column prop="contact" label="联系人" width="100" />
      <el-table-column prop="phone" label="电话" width="130" />
      <el-table-column prop="ports" label="主营航线/港区" min-width="160" show-overflow-tooltip />
      <el-table-column prop="paymentTerms" label="结算条款" width="110" />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑供应商' : '新增供应商'" width="640px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="供应商名称" required><el-input v-model="form.name" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="编码"><el-input v-model="form.code" :disabled="!!form.id" placeholder="留空自动生成" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="类型"><el-select v-model="form.category" style="width:100%"><el-option v-for="(t,k) in SUPPLIER_CATEGORY" :key="k" :label="t" :value="k" /></el-select></el-form-item></el-col>
          ...
          <el-col :span="12"><el-form-item label="主营航线/港区"><el-input v-model="form.ports" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="合同编号"><el-input v-model="form.contractNo" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="结算条款"><el-input v-model="form.paymentTerms" /></el-form-item></el-col>
    ...
    <!-- 批量导入 -->
    <el-dialog v-model="importer.visible.value" title="批量导入供应商" width="520px" @closed="importer.close()">
      <el-alert type="info" :closable="false" show-icon style="margin-bottom:14px">
        请先下载模板，按模板格式填写后上传 Excel 文件（.xlsx）。编码留空将自动生成。
      </el-alert>
...
import { supplierAPI, supplierImportAPI, supplierImportTemplateAPI } from '@/api';
import { SUPPLIER_CATEGORY, dictText } from '@/utils/dicts';
...
const list = ref([]);
const total = ref(0);
const query = reactive({ page: 1, pageSize: 10, keyword: '', category: '', status: '' });
const dialogVisible = ref(false);
const form = ref({});
const multiple = ref([]);

const importer = useImport({
  importAPI: supplierImportAPI,
  templateAPI: supplierImportTemplateAPI,
  refresh: () => load(1),
  fileName: '供应商',
});
...
async function batchStatus(status) {
  await ElMessageBox.confirm(`确认将选中的 ${selectedIds().length} 家供应商${status === 'active' ? '启用' : '停用'}？`, '批量操作', { type: 'warning' });
  await supplierAPI.batchUpdate(selectedIds(), { status });
...
async function load(page) {
  ...
  loading.value = true;
  try {
    const data = await supplierAPI.list(query);
...
function openDialog(row) {
  form.value = row ? { ...row } : { category: 'carrier', status: 'active' };
  dialogVisible.value = true;
}

async function save() {
  if (!form.value.name) return ElMessage.warning('请填写供应商名称');
  saving.value = true;
  try {
...
async function toggleStatus(row) {
  await supplierAPI.update(row.id, { ...row, status: row.status === 'active' ? 'inactive' : 'active' });

...
│  后端 (Express, :3000)                                                    │
│  ┌────────── 路由层 ──────────┐  ┌────────── 中间件 ──────────┐            │
│  │ 30+ 控制器 · 统一 CRUD 工厂 │  │ auth(JWT) · RBAC guard      │            │
...
│  └─────────────────────────────┘  └─────────────────────────────┘          │
│  ┌────────── 数据层 ───────────┐  ┌────────── 外部对接 ──────────┐          │
│  │ 35+ 模型 · Sequelize ORM     │  │ 12 适配器: 船期/汇率/港口/   │          │
...
| 领域 | 规模 | 说明 |
| --- | --- | --- |
| 数据模型 | 35+ 张表 | 订单为中枢，订舱/报关/单证/跟踪/财务/发票/放单/预警/EDI/收款全 `belongsTo(Order)` |
| 后端控制器 | 32 个 | 通用 CRUD 工厂 + 业务控制器 |
| 外部适配器 | 12 个 | shipSchedule · exchangeRate · port(含沪/甬/青) · customs · aisTracking · yardQingdao · freightRate · finance · usdPay |
| 前端视图 | 28 个 | 含客户门户、待办工作台、系统管理 |
